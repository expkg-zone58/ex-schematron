...
let $doc := <book xmlns="http://docbook.org/ns/docbook">
<title>Some Title</title>
<chapter>
<title>Some Chapter Title</title>
<para>...</para>
</chapter>
</book>

let $sch := <s:schema xmlns:s="http://purl.oclc.org/dsdl/schematron"
...
return
  let $output := sch:validate-document($doc, $sch)/svrl:schematron-output
  return
    if ($output/svrl:fired-rule[@context="/db:book"]
        and $output/svrl:failed-assert[@test="@version"])
    then
      "PASS"
    else
      <error>
        <message>FAIL: Schematron did not run successfully?</message>
